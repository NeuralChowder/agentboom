"""`agentloom upgrade` — sync an agent's managed base files with this agentloom.

Managed files are re-rendered from the template using the variables stored
in .agentloom.json. Files the agent modified locally are skipped (reported)
...
def _base_managed_rels(template: str) -> set:
    ...
    for src in source.rglob("*"):
        if not src.is_file() or src.name == ".agentloom-template.json":
            continue
...
def run(args) -> dict:
    ...
    if registry is None:
        raise UpgradeError(
            f"No .agentloom.json in {agent_dir} — not an agentloom-managed agent."
        )
...
def _rendered_sha(src: Path, variables: dict, rel: str) -> str:
    from agentloom.render import is_probably_binary, render_text

    data = src.read_bytes()
    if is_probably_binary(data):
        from agentloom.registry import sha256_file as _f
