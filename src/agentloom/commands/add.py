"""`agentloom add skill|miniapp` — scaffold new capabilities inside an agent."""
import json
import re
from pathlib import Path

from agentloom.registry import load_registry
...
def _agent_dir(args) -> Path:
    ...
    if not (agent_dir / ".agentloom.json").is_file() and not (
        agent_dir / ".qwen-docker"
    ).is_dir():
        raise AddError(
            f"{agent_dir} does not look like an agent project "
            "(no .agentloom.json or .qwen-docker). Run from inside an agent "
            "or pass --dir."
        )
    return agent_dir
