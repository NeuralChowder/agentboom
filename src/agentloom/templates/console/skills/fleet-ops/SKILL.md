---
name: fleet-ops
description: Procedures for operating the agentloom fleet — create agents, sync the base, install packages, triage drift and health. Use for any multi-agent management task.
---
...
1. `agentloom fleet --json` — drift, validate errors, base version.
...
1. The SDK: bump the pin (`agentloom_sdk @ .../vX.Y.Z/...whl`) in each
   agent's `platform/requirements.txt`/Dockerfile, rebuild, run the
   agent's tests, redeploy only after green.
2. Template glue: `agentloom upgrade` per agent (check-only first;
...
- Repeated integration (channels, docs, credential stores) → check
  `agentloom packages`; if missing, build a new package in the agentloom
  repo (`templates/packages/<name>/`) rather than copy-pasting into agents.
- Agent-specific procedure → `agentloom add skill` inside that agent.
