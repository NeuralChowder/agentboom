"""agentloom command-line interface.
...
from agentloom.commands import DEFAULT_TEMPLATE, list_templates
from agentloom.render import TemplateError
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"agentloom {__version__}")
    ...
    p = sub.add_parser("upgrade", parents=[common],
                       help="sync managed base files with this agentloom version")
    p.add_argument("dir", nargs="?", default=".", help="agent directory (default: cwd)")
    ...
    p = sub.add_parser("list", parents=[common],
                       help="discover agentloom agents under a directory")
...
def _print_human(command: str, result: dict) -> None:
    if command == "init":
        ...
        print(f"agentloom doctor: {'PASS' if result['ok'] else 'FAIL'}")
    elif command == "list":
        if not result["agents"]:
            print(f"No agentloom agents found under {result['parent']}")
        ...
        print(f"agentloom selfcheck: {'PASS' if result['ok'] else 'FAIL'}")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"agentloom: {exc}", file=sys.stderr)
        return 1
