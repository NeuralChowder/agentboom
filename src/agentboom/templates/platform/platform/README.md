...
<!-- agentboom:managed — upgraded by `agentboom upgrade`; local edits become drift. -->
...
- `agent` jobs run one Qwen Code agent turn with the `prompt` — serialized
  through `agentboom_sdk.agent`'s task queue.
...
| Module | Purpose |
|---|---|
| `agentboom_sdk.config` | env parsing (`env`, `env_int`, `env_bool`, `require`) |
| `agentboom_sdk.log` | logging setup (`get_logger`) |
| `agentboom_sdk.db` | SQLite (WAL) + migration runner — data lives on the named volume |
| `agentboom_sdk.agent` | `ask()` / `ask_json()` — run agent turns via `qwen serve` HTTP |
| `agentboom_sdk.llm` | `complete()` / `complete_json()` — one-shot completions |
| `agentboom_sdk.cron` | cron parsing + `next_cron_time` |
| `agentboom_sdk.task_queue` | bounded priority queue serializing LLM traffic |
| `agentboom_sdk.events` | `publish()` / `subscribe()` in-process event bus |
| `agentboom_sdk.untrusted` | `wrap()` — fence external content before any model sees it |
...
- **External content is data, not instructions.** Fence it with
  `agentboom_sdk.untrusted.wrap` before any model sees it.
