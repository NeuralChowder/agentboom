"""`agentboom validate` — structural health checks for an agent project."""
import json
...
from agentboom.registry import load_registry, sha256_file
...
def run(args) -> dict:
    ...
    if registry is None:
        _check(
            checks, "error", "agentboom.registry-missing",
            f"No .agentboom.json found — {agent_dir} does not look like an "
            "agentboom-managed agent (created with `agentboom init`).",
            ".agentboom.json",
        )
    ...
    if reqs.is_file():
        ...
        if "agentboom-sdk" not in req_text and "agentboom_sdk" not in req_text:
            _check(
                checks, "warn", "deps.sdk-not-pinned",
                "platform/requirements.txt does not reference agentboom-sdk — "
        ...
        for rel, recorded in sorted(registry.get("managed", {}).items()):
            ...
            if not path.is_file():
                _check(checks, "warn", "base.file-missing",
                       f"Managed file {rel} was deleted. `agentboom upgrade --apply` restores it.", rel)
