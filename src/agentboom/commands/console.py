"""`agentboom console` — drop into a pre-configured operator session.

Materializes the boomkeeper workspace (operator manual + fleet-ops skill
+ a live snapshot of the fleet) under $AGENTBOOM_HOME/console and execs
`qwen` (Qwen Code) inside it. The session arrives already knowing how to
use every agentboom command and how to work on any registered agent —
the independence guarantee holds throughout: the console only ever acts
through agentboom commands and git/docker on the agent repos.
"""
import os
import shutil
...
def console_dir() -> Path:
    return fleet_reg.agentboom_home() / CONSOLE_DIR_NAME
...
def _workspace_files() -> dict:
    ...
    for row in rows:
        ...
        if not row.get("ok"):
            ...
            continue
        lines.append(f"- path: `{row['path']}`")
        lines.append(f"- base: agentboom {row['base_version']} "
...
def run(args) -> dict:
    ...
    # Operator manual + skills are owned by agentboom and refreshed each
