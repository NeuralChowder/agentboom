"""`agentboom fleet` — the operator's view of every known agent."""
import argparse
...
def _row(entry: dict) -> dict:
    ...
    if registry is None:
        row["error"] = "no .agentboom.json (run: agentboom adopt)"
        return row
...
def run_remove(args) -> dict:
    ...
    raise FleetError(
        f"'{args.name}' is not in the fleet (agentboom fleet to list)."
    )
