"""`agentboom adopt` — bring an existing agent under base management.
...
`agentboom upgrade` behaves exactly as it does for an initialized agent.
"""
import argparse
import re
...
def run(args) -> dict:
    ...
    if load_registry(agent_dir) is not None:
        raise AdoptError(
            f"{agent_dir} already has a .agentboom.json — "
            "use `agentboom upgrade` to sync it."
        )
    ...
    return {
        ...
        "next": [
            "agentboom validate   # structural health",
            "agentboom upgrade    # check-only sync of the managed subset",
        ],
    }
