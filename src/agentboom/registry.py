"""The per-agent registry (.agentboom.json) and managed-file bookkeeping.

A generated agent records which of its files are *managed* (owned by the
agentboom base, upgradable with `agentboom upgrade`) together with the
...
REGISTRY_NAME = ".agentboom.json"
TEMPLATE_META_NAME = ".agentboom-template.json"
