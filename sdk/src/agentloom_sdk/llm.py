...
Two different things get confused here, so be explicit about which you want:
  ...
  * **Agent work** — something needing tools, files or judgement across
    ...
    from agentloom_sdk.llm import complete, complete_json

    verdict = await complete_json(prompt)

Serialization
-------------
When the shared task queue has been started (`agentloom_sdk.task_queue`),
...
from agentloom_sdk.task_queue import TaskPriority, queue as _task_queue
